...
namespace fastllm {
    ...
    static int Glm5MoeDsaGetInt(const WeightMap &weight, const std::string &key, int fallback) {
    ...
    static float Glm5MoeDsaGetFloat(const WeightMap &weight, const std::string &key, float fallback) {
        auto it = weight.dicts.find(key);
        if (it == weight.dicts.end()) {
            return fallback;
        }
    ...
    static std::string Glm5MoeDsaGetString(const WeightMap &weight, const std::string &key,
    ...
    static bool Glm5MoeDsaShouldPrintLogits() {
        return GetFastllmEnv().printLogits;
    }

    static bool Glm5MoeDsaShouldPrintIntermediate() {
        const char *value = std::getenv("FASTLLM_PRINT_INTERMEDIATE");
        return value != nullptr && value[0] != '\0' && value[0] != '0';
    }

    static bool Glm5MoeDsaDisableSharedGateupMerge() {
        const char *value = std::getenv("FASTLLM_GLM_MOE_DSA_DISABLE_SHARED_GATEUP_MERGE");
        return value != nullptr && value[0] != '\0' && value[0] != '0';
    }
    ...
    static void Glm5MoeDsaMaybeMarkGlmDenseFp32Dequant(WeightMap &weight, bool isGlmMoeDsa) {
        ...
        for (auto &it : weight.weight) {
            ...
            if (name.find(".mlp.gate_proj.") != std::string::npos ||
                ...
                name.find(".mlp.down_proj.") != std::string::npos) {
                it.second.forceGGUFFp32Dequant = true;
            }
        }
    }

    static bool Glm5MoeDsaIsExpertWeightName(const std::string &name) {
        return name.find(".mlp.experts.") != std::string::npos ||
               name.find(".mlp.shared_experts.") != std::string::npos;
    }

    static void Glm5MoeDsaDisableGGUFExpertRepack(WeightMap &weight) {
        for (auto &it : weight.weight) {
            if (it.second.dataType == DataType::DATA_GGUF_FORMAT &&
                Glm5MoeDsaIsExpertWeightName(it.first)) {
                it.second.disableGGUFRepack = true;
            }
        }
    }

    static bool Glm5MoeDsaShouldPrintLayer(int layer) {
        const char *value = std::getenv("FASTLLM_PRINT_LAYER");
        if (value == nullptr) {
            return false;
        }
        std::string layers = value;
        if (layers == "all" || layers == "*") {
            return true;
        }
        size_t start = 0;
        while (start <= layers.size()) {
            size_t end = layers.find(',', start);
            std::string item = layers.substr(start, end == std::string::npos ? std::string::npos : end - start);
            if (!item.empty() && atoi(item.c_str()) == layer) {
                return true;
            }
    ...
    static bool Glm5MoeDsaShouldForceCpuOProj(int layer) {
        const char *value = std::getenv("FASTLLM_FORCE_CPU_O_PROJ_LAYER");
    ...
    static bool Glm5MoeDsaShouldForceCpuDenseFFN(int layer) {
        const char *value = std::getenv("FASTLLM_FORCE_CPU_DENSE_FFN_LAYER");
        if (value == nullptr || value[0] == '\0' || value[0] == '0') {
            return false;
        }
        std::string layers = value;
    ...
    static void Glm5MoeDsaCopyGGUFWeightForCpu(Data &dst, Data &src) {
        dst.CopyFrom(src);
        dst.ggmlTensor = src.ggmlTensor;
        dst.disableGGUFRepack = true;
        dst.ToDevice(DataDevice::CPU);
    }

    static bool Glm5MoeDsaShouldDisableMergeMLA(int layer) {
        const char *value = std::getenv("FASTLLM_DISABLE_MERGE_MLA_LAYER");
        if (value == nullptr || value[0] == '\0' || value[0] == '0') {
            return false;
        }
        std::string layers = value;
    ...
    static void Glm5MoeDsaPrintDataPrefix(Data &data, const char *tag) {
        if (!Glm5MoeDsaShouldPrintIntermediate()) {
            return;
        }
        Data tmp;
        tmp.CopyFrom(data);
        ...
        int hidden = tmp.dims.empty() ? (int)tmp.Count(0) : tmp.dims.back();
        int printLimit = 16;
        const char *limitEnv = std::getenv("FASTLLM_PRINT_VECTOR_LIMIT");
        if (limitEnv != nullptr && limitEnv[0] != '\0') {
            printLimit = atoi(limitEnv);
        }
        int limit = printLimit <= 0 ? hidden : std::min(printLimit, hidden);
        int offset = std::max(0, (int)tmp.Count(0) - hidden);
        printf("%s dims", tag);
        for (int dim : tmp.dims) {
            printf(" %d", dim);
        }
        printf(" lastvec_first%d", limit);
        float *ptr = (float*) tmp.cpuData;
        for (int i = 0; i < limit; i++) {
            printf(" %.8g", ptr[offset + i]);
        }
        printf("\n");
        fflush(stdout);
    }

    static void Glm5MoeDsaPrintLayerData(Data &data, int layer, const char *tag) {
        if (!Glm5MoeDsaShouldPrintLayer(layer)) {
            return;
        }
        std::string layerTag = std::string(tag) + "-" + std::to_string(layer);
        Glm5MoeDsaPrintDataPrefix(data, layerTag.c_str());
    }

    static void Glm5MoeDsaPrintLayerHead0LastToken(Data &data, int layer, const char *tag) {
        if (!Glm5MoeDsaShouldPrintLayer(layer)) {
            return;
        }
        Data tmp;
        ...
        ToDataType(tmp, DataType::FLOAT32);
        tmp.ToDevice(DataDevice::CPU);
        if (tmp.dims.size() < 3 || tmp.cpuData == nullptr) {
            Glm5MoeDsaPrintLayerData(tmp, layer, tag);
            return;
        }

        int hidden = tmp.dims.back();
        int printLimit = 16;
        const char *limitEnv = std::getenv("FASTLLM_PRINT_VECTOR_LIMIT");
        if (limitEnv != nullptr && limitEnv[0] != '\0') {
            printLimit = atoi(limitEnv);
        ...
        int limit = printLimit <= 0 ? hidden : std::min(printLimit, hidden);
        int token = std::max(0, tmp.dims[1] - 1);
        int offset = token * hidden;
        if (tmp.dims.size() == 4) {
            offset = ((0 * tmp.dims[1] + token) * tmp.dims[2]) * hidden;
        }
        std::string layerTag = std::string(tag) + "-" + std::to_string(layer);
        printf("%s dims", layerTag.c_str());
        for (int dim : tmp.dims) {
            printf(" %d", dim);
        }
        printf(" head0_lastvec_first%d", limit);
        float *ptr = (float*) tmp.cpuData;
        for (int i = 0; i < limit; i++) {
            printf(" %.8g", ptr[offset + i]);
        }
    ...
    static bool Glm5MoeDsaIsEmptyTensor(const Data &data) {
        return data.dims.empty() && data.expansionDims.empty();
    }

    static void Glm5MoeDsaCatDirectAs(Data &dst, Data &src, int axis) {
        if (Glm5MoeDsaIsEmptyTensor(dst)) {
        ...
        Data typedSrc;
        ToDataType(src, typedSrc, dst.dataType);
        CatDirect(dst, typedSrc, axis);
    }

    static void Glm5MoeDsaPrintLayerDataSum(Data &a, Data &b, int layer, const char *tag) {
        if (!Glm5MoeDsaShouldPrintLayer(layer)) {
            return;
        }
        Data cpuA, cpuB;
        cpuA.CopyFrom(a);
        cpuB.CopyFrom(b);
        ToDataType(cpuA, DataType::FLOAT32);
        ToDataType(cpuB, DataType::FLOAT32);
        cpuA.ToDevice(DataDevice::CPU);
        cpuB.ToDevice(DataDevice::CPU);
        if (cpuA.Count(0) != cpuB.Count(0)) {
            return;
        }
        float *aData = (float*) cpuA.cpuData;
        float *bData = (float*) cpuB.cpuData;
        int count = (int) cpuA.Count(0);
        for (int i = 0; i < count; i++) {
            aData[i] += bData[i];
        }
        Glm5MoeDsaPrintLayerData(cpuA, layer, tag);
    }

    static void Glm5MoeDsaPrintExpertChoice(Data &index, Data &score, int layer, const char *tag) {
        if (!Glm5MoeDsaShouldPrintLayer(layer)) {
            return;
        }
        if (index.dataType != DataType::INT32 || score.dataType != DataType::FLOAT32) {
            return;
        }
        Data cpuIndex, cpuScore;
        cpuIndex.CopyFrom(index);
        cpuScore.CopyFrom(score);
        cpuIndex.ToDevice(DataDevice::CPU);
        ToDataType(cpuScore, DataType::FLOAT32);
        cpuScore.ToDevice(DataDevice::CPU);
        if (cpuIndex.dims.size() < 2 || cpuScore.dims.size() < 2) {
            return;
        }
        int rows = cpuIndex.Count(0) / cpuIndex.dims.back();
        int topk = cpuIndex.dims.back();
        int row = std::max(0, rows - 1);
        int32_t *indexData = (int32_t*) cpuIndex.cpuData;
        float *scoreData = (float*) cpuScore.cpuData;
        printf("%s-%d dims", tag, layer);
        for (int dim : cpuIndex.dims) {
            printf(" %d", dim);
        }
        printf(" lastrow_top%d", topk);
        for (int i = 0; i < topk; i++) {
            int offset = row * topk + i;
            printf(" %d:%.8g", indexData[offset], scoreData[offset]);
        }
        printf("\n");
        fflush(stdout);
    }

    static void Glm5MoeDsaPrintOptionalWeight(WeightMap &weight, const std::string &name, int layer, const char *tag) {
        if (!Glm5MoeDsaShouldPrintIntermediate() || !Glm5MoeDsaShouldPrintLayer(layer)) {
            return;
        }
        auto it = weight.weight.find(name);
        std::string layerTag = std::string(tag) + "-" + std::to_string(layer);
        if (it == weight.weight.end()) {
            printf("%s missing %s\n", layerTag.c_str(), name.c_str());
            fflush(stdout);
            return;
        }
        Glm5MoeDsaPrintDataPrefix(it->second, layerTag.c_str());
    }

    static void Glm5MoeDsaPrintRouterBiasedTopK(Data &probs, Data *bias, int layer, const char *tag, int topK = 16) {
        if (!Glm5MoeDsaShouldPrintIntermediate() || !Glm5MoeDsaShouldPrintLayer(layer)) {
            return;
        }
        Data cpuProbs;
        cpuProbs.CopyFrom(probs);
        ToDataType(cpuProbs, DataType::FLOAT32);
        cpuProbs.ToDevice(DataDevice::CPU);
        if (cpuProbs.dims.empty()) {
            return;
        }
        int experts = cpuProbs.dims.back();
        int rows = cpuProbs.Count(0) / experts;
        int row = std::max(0, rows - 1);
        Data cpuBias;
        float *biasData = nullptr;
        if (bias != nullptr && bias->dims.size() > 0) {
            cpuBias.CopyFrom(*bias);
            ToDataType(cpuBias, DataType::FLOAT32);
            cpuBias.ToDevice(DataDevice::CPU);
            biasData = (float*) cpuBias.cpuData;
        }
        float *probData = (float*) cpuProbs.cpuData + (size_t)row * experts;
        std::vector<std::pair<float, int> > values;
        values.reserve(experts);
        for (int i = 0; i < experts; i++) {
            float biased = probData[i] + (biasData == nullptr ? 0.0f : biasData[i]);
            values.push_back(std::make_pair(-biased, i));
        }
        topK = std::min(topK, experts);
        std::partial_sort(values.begin(), values.begin() + topK, values.end());
        printf("%s-%d biased_top%d", tag, layer, topK);
        for (int i = 0; i < topK; i++) {
            int idx = values[i].second;
            float biasValue = biasData == nullptr ? 0.0f : biasData[idx];
            printf(" %d:%.8g/%.8g/%.8g", idx, probData[idx], biasValue, probData[idx] + biasValue);
        }
        printf("\n");
        fflush(stdout);
    }

    static void Glm5MoeDsaPrintTopKRows(const char *tag, const float *topkData, int batch,
                                        int rowsPerBatch, int topK) {
        printf("%s top%d logits:\n", tag, topK);
        for (int b = 0; b < batch; b++) {
            int rowId = b * rowsPerBatch + rowsPerBatch - 1;
            const float *row = topkData + (size_t)rowId * topK * 2;
            printf("  batch %d:", b);
    ...
    static void Glm5MoeDsaPrintLogitsTopK(Data &logits, int batch, const char *tag) {
        if (!Glm5MoeDsaShouldPrintLogits() || logits.dims.empty() || batch <= 0) {
            return;
        }
        ...
        int rows = (int)(logits.Count(0) / vocabSize);
        int rowsPerBatch = std::max(1, rows / batch);
        Data topk;
        TopK(logits, topk, topK);
        topk.ToDevice(DataDevice::CPU);
        Glm5MoeDsaPrintTopKRows(tag, (const float*)topk.cpuData, batch, rowsPerBatch, topK);
    }
    ...
    Glm5MoeDsaModel::Glm5MoeDsaModel() {
        this->model_type = "glm_moe_dsa";
        this->model_struct = "glm_moe_dsa";
    ...
    std::map <std::string, std::vector <std::pair <std::string, DataType> > >
            Glm5MoeDsaModel::GetTensorMap(const std::vector <std::string> &tensorNames) {
        auto ret = basellm::GetTensorMap(tensorNames);
        if (this->model_type != "glm_moe_dsa") {
            return ret;
        }
        const std::string layerPrefix = "model.layers.";
        for (auto &name : tensorNames) {
            bool skip = name.find(".self_attn.indexer.") != std::string::npos;
            if (!skip && name.compare(0, layerPrefix.size(), layerPrefix) == 0) {
                int pos = (int)layerPrefix.size();
                int layerId = 0;
                bool hasLayerId = false;
                while (pos < (int)name.size() && name[pos] >= '0' && name[pos] <= '9') {
                    hasLayerId = true;
                    layerId = layerId * 10 + name[pos] - '0';
                    pos++;
                }
                skip = hasLayerId && pos < (int)name.size() && name[pos] == '.' && layerId >= this->block_cnt;
            }
            if (skip) {
                ret[name].clear();
            }
    ...
    void Glm5MoeDsaModel::InitParams() {
        basellm::InitParams();
        bool isGlmMoeDsa = this->model_type == "glm_moe_dsa";
        Glm5MoeDsaMaybeMarkGlmDenseFp32Dequant(this->weight, isGlmMoeDsa);
        rope_scaling_type = Glm5MoeDsaGetString(this->weight, "rope_scaling.type",
                            Glm5MoeDsaGetString(this->weight, "rope_parameters.rope_type", "yarn"));
        if (!isGlmMoeDsa) {
            AssertInFastLLM(rope_scaling_type == "yarn", "Fastllm.Glm5MoeDsa: Only support ropescaling.type = yarn.");
        }

        rope_scaling_beta_fast = Glm5MoeDsaGetInt(this->weight, "rope_scaling.beta_fast", 32);
        rope_scaling_beta_slow = Glm5MoeDsaGetInt(this->weight, "rope_scaling.beta_slow", 1);
        rope_scaling_original_max_position_embeddings =
                Glm5MoeDsaGetInt(this->weight, "rope_scaling.original_max_position_embeddings", 4096);
        rope_scaling_mscale = Glm5MoeDsaGetFloat(this->weight, "rope_scaling.mscale", isGlmMoeDsa ? 1.0f : 0.707f);
        rope_scaling_mscale_all_dim =
                Glm5MoeDsaGetFloat(this->weight, "rope_scaling.mscale_all_dim", isGlmMoeDsa ? 1.0f : 0.707f);

        routed_scaling_factor = Glm5MoeDsaGetFloat(this->weight, "routed_scaling_factor", 1.0f);
        max_position_embeddings = Glm5MoeDsaGetInt(this->weight, "max_position_embeddings", max_positions);
        rope_theta = Glm5MoeDsaGetInt(this->weight, "rope_theta",
                     Glm5MoeDsaGetInt(this->weight, "rope_parameters.rope_theta", 10000));
        q_lora_rank = Glm5MoeDsaGetInt(this->weight, "q_lora_rank", 0);
        qk_rope_head_dim = Glm5MoeDsaGetInt(this->weight, "qk_rope_head_dim", rotary_dim);
        kv_lora_rank = Glm5MoeDsaGetInt(this->weight, "kv_lora_rank", 0);
        v_head_dim = Glm5MoeDsaGetInt(this->weight, "v_head_dim", head_dim);
        qk_nope_head_dim = Glm5MoeDsaGetInt(this->weight, "qk_nope_head_dim",
                                           Glm5MoeDsaGetInt(this->weight, "head_dim", head_dim) - qk_rope_head_dim);
        q_head_dim = qk_nope_head_dim + qk_rope_head_dim;

        n_shared_experts = Glm5MoeDsaGetInt(this->weight, "n_shared_experts", 0);
        num_experts = Glm5MoeDsaGetInt(this->weight, "n_routed_experts", 0);
        num_experts_per_tok = Glm5MoeDsaGetInt(this->weight, "num_experts_per_tok", 1);
        norm_topk_prob = Glm5MoeDsaGetString(this->weight, "norm_topk_prob", "false") == "true";
        ...
        rope_base = Glm5MoeDsaGetFloat(this->weight, "rope_theta",
                    Glm5MoeDsaGetFloat(this->weight, "rope_parameters.rope_theta", rope_base));
        rope_factor = Glm5MoeDsaGetFloat(this->weight, "rope_scaling.factor", rope_factor);
        ...
        const bool disableSharedGateupMerge = Glm5MoeDsaDisableSharedGateupMerge();
        for (int i = 0; i < block_cnt; i++) {
            for (int j = -1; j < this->num_experts; j++) {
                ...
                if (!(j == -1 && disableSharedGateupMerge)) {
    ...
    void Glm5MoeDsaModel::OnModelWeightsLoaded() {
        Glm5MoeDsaMaybeMarkGlmDenseFp32Dequant(this->weight, this->model_type == "glm_moe_dsa");
        Glm5MoeDsaDisableGGUFExpertRepack(this->weight);
    }

    static float glm5_moe_dsa_yarn_find_correction_dim(int num_rotations, int dim, float base, int max_position_embeddings) {
        return (dim * log(max_position_embeddings / (num_rotations * 2 * M_PI))) / (2 * log(base));
    }

    static void glm5_moe_dsa_yarn_find_correction_range(int low_rot, int high_rot, int dim, float base, int max_position_embeddings, int &low, int &high) {
        low = (int)(floor(glm5_moe_dsa_yarn_find_correction_dim(low_rot, dim, base, max_position_embeddings)) + 1e-5);
        high = (int)(ceil(glm5_moe_dsa_yarn_find_correction_dim(high_rot, dim, base, max_position_embeddings)) + 1e-5);
        low = std::max(low, 0);
        high = std::min(high, dim - 1);
    }

    static float glm5_moe_dsa_yarn_get_mscale(float scale, float mscale) {
        if (scale <= 1) {
            return 1.0;
        }
    ...
    static std::vector <float> glm5_moe_dsa_yarn_linear_ramp_mask(float min, float max, int dim) {
    ...
    std::pair<std::vector<float>, std::vector<float>> Glm5MoeDsaModel::UpdateRotaryPosEmb(float base, float factor, int seqLen) {
        int dim = rotary_dim;
        ...
        int low, high;
        glm5_moe_dsa_yarn_find_correction_range (
        ...
        std::vector <float> invFreqMask = glm5_moe_dsa_yarn_linear_ramp_mask(low, high, dim / 2);
        ...
        float _mscale = glm5_moe_dsa_yarn_get_mscale(rope_factor, rope_scaling_mscale) / glm5_moe_dsa_yarn_get_mscale(rope_factor, rope_scaling_mscale_all_dim);
    ...
    int Glm5MoeDsaModel::Forward(const fastllm::Data &inputIds, const fastllm::Data &attentionMask,
    ...
    static std::vector <int> Glm5MoeDsaGumbelMaxTrick(Data &logits, int batch) {
        std::vector <int> ret;
    ...
    std::vector <int> Glm5MoeDsaModel::ForwardBatchV2(int batch, const fastllm::Data &inputIds, const fastllm::Data &attentionMask,
        const fastllm::Data &positionIds, std::vector<std::pair<Data, Data>> &pastKeyValues,
        const GenerationConfig &generationConfig, const LastTokensManager &lastTokens,
        ...
        Data curInput, curOutput, moeInputTemp, moeOutputTemp;
        ...
        for (int i = 0; i < block_cnt; i++) {
            ...
            Glm5MoeDsaPrintLayerData(attenInput, i, "Glm5MoeDsa Forward attn_norm");
            ...
            bool usePagedMLA = attentionMask.dims.size() == 0 &&
                               (pastKeyValues[i].first.dims.empty() ||
                                pastKeyValues[i].first.isPagedKVCache);
            if (usePagedMLA) {
                ...
                if (Glm5MoeDsaNeedPermuteKVB(kv0, kv_lora_rank, qk_nope_head_dim)) {
                ...
                Glm5MoeDsaCatDirectAs(pastKey, k, 1);
                Glm5MoeDsaCatDirectAs(pastValue, v, 1);
                ...
                if (attentionMask.dims.size() == 0 && b == 1 && s > 1 && t > 1) {
                    ...
                    for (int row = 0; row < s; row++) {
                        for (int col = t - s + row + 1; col < t; col++) {
                            vmasks[row * t + col] = 1.0;
                        }
                    }
                ...
                if (!Glm5MoeDsaShouldDisableMergeMLA(i)) {
            ...
            Glm5MoeDsaPrintLayerData(attenLastOutput, i, "Glm5MoeDsa Forward attn_out");

            AddTo(hiddenStates, attenLastOutput);
            Glm5MoeDsaPrintLayerData(hiddenStates, i, "Glm5MoeDsa Forward ffn_inp");
            RMSNorm(hiddenStates, this->weight["model.layers." + std::to_string(i) + ".post_attention_layernorm.weight"], rms_norm_eps, attenInput);
            Glm5MoeDsaPrintLayerData(attenInput, i, "Glm5MoeDsa Forward ffn_norm");
            // 2. moe mlp
            if (weight.weight.find("model.layers." + std::to_string(i) + ".mlp.gate_proj.weight") != weight.weight.end()) {
                if (Glm5MoeDsaShouldForceCpuDenseFFN(i)) {
                    Data cpuAttenInput, cpuGateWeight, cpuUpWeight, cpuDownWeight;
                    cpuAttenInput.CopyFrom(attenInput);
                    cpuAttenInput.ToDevice(DataDevice::CPU);
                    Glm5MoeDsaCopyGGUFWeightForCpu(cpuGateWeight, weight["model.layers." + std::to_string(i) + ".mlp.gate_proj.weight"]);
                    Glm5MoeDsaCopyGGUFWeightForCpu(cpuUpWeight, weight["model.layers." + std::to_string(i) + ".mlp.up_proj.weight"]);
                    Glm5MoeDsaCopyGGUFWeightForCpu(cpuDownWeight, weight["model.layers." + std::to_string(i) + ".mlp.down_proj.weight"]);
                    Linear(cpuAttenInput, cpuGateWeight, Data(), w1);
                    Silu(w1, w1);
                    Linear(cpuAttenInput, cpuUpWeight, Data(), w3);
                    MulTo(w1, w3);
                    Linear(w1, cpuDownWeight, Data(), w2);
                    w2.ToDevice(hiddenStates.dataDevice, hiddenStates.dataDeviceIds);
                } else if (CanRunLinearEx(LinearExType::ExSilu)) {
                ...
                Glm5MoeDsaPrintLayerData(w2, i, "Glm5MoeDsa Forward ffn_out");
                ...
                Glm5MoeDsaPrintOptionalWeight(this->weight, gateBiasName, i, "Glm5MoeDsa Forward ffn_moe_gate_bias");

                int batch = attenInput.dims[0], len = attenInput.dims[1];
                attenInput.Reshape({batch * len, attenInput.dims[2]});

                if (cudaSe) {
                    std::string sharedGateupName = "model.layers." + std::to_string(i) + ".mlp.shared_experts.gateup_proj.weight";
                    auto sharedGateupIt = weight.weight.find(sharedGateupName);
                    if (sharedGateupIt != weight.weight.end() && sharedGateupIt->second.dims.size() > 0) {
                        Linear(attenInput, weight[sharedGateupName], Data(), ww3);
                        Swiglu(ww3, ww1);
                    } else {
                        Linear(attenInput, weight["model.layers." + std::to_string(i) + ".mlp.shared_experts.gate_proj.weight"], Data(), ww3);
                        Silu(ww3, ww1);
                        Linear(attenInput, weight["model.layers." + std::to_string(i) + ".mlp.shared_experts.up_proj.weight"], Data(), ww3);
                        MulTo(ww1, ww3);
                    }
                    Linear(ww1, weight["model.layers." + std::to_string(i) + ".mlp.shared_experts.down_proj.weight"], Data(), moeFinal2);
                    Glm5MoeDsaPrintLayerData(moeFinal2, i, "Glm5MoeDsa Forward ffn_shexp");
                ...
                Glm5MoeDsaPrintLayerData(routerLogits, i, "Glm5MoeDsa Forward ffn_moe_probs");
                Data *gateBias = weight.weight.find(gateBiasName) != weight.weight.end() ? &weight[gateBiasName] : nullptr;
                Glm5MoeDsaPrintRouterBiasedTopK(routerLogits, gateBias, i, "Glm5MoeDsa Forward ffn_moe_probs_biased");
                ...
                if (weight.weight.find("model.layers." + std::to_string(i) + ".mlp.experts.0.gateup_proj.weight") != weight.weight.end()
                    ...
                    MergeMOEBlock(&attenInput, &expertIndex, &expertScore,
                        ...
                        1.0f, &moeFinal, i,
                        attenInput.dataType, this->moeAtype,
                        &moeInputTemp, &moeOutputTemp);
                    if (cudaSe) {
                        Glm5MoeDsaPrintLayerData(moeFinal, i, "Glm5MoeDsa Forward ffn_moe_out");
                    }
                } else {
                    ...
                    for (int b = 0; b < batch * len; b++) {
                        ...
                        Glm5MoeDsaCatDirectAs(moeFinal, moePart, 0);
                    }
                ...
                if (!cudaSe) {
                    Glm5MoeDsaPrintLayerData(moeFinal, i, "Glm5MoeDsa Forward ffn_out");
                }
                ...
                if (cudaSe) {
                    ...
                    Glm5MoeDsaPrintLayerDataSum(moeFinal, moeFinal2, i, "Glm5MoeDsa Forward ffn_out");
                    AddTo(hiddenStates, moeFinal2);
                }
            }
            Glm5MoeDsaPrintLayerData(hiddenStates, i, "Glm5MoeDsa Forward l_out");
        }
        ...
        std::vector <int> lastRet;
        {
            ...
            Glm5MoeDsaPrintDataPrefix(hiddenStates, "Glm5MoeDsa Forward result_norm");
            Linear(hiddenStates, weight["lm_head.weight"], Data(), logits);
            ToDataType(logits, DataType::FLOAT32);
            ResetLogitsOfEOS(batch, &logits, pastKeyValues, generationConfig);
            Glm5MoeDsaPrintLogitsTopK(logits, batch, "Glm5MoeDsa Forward");
            if (Glm5MoeDsaGetString(this->weight, "model_type") != "glm_moe_dsa") {
    ...
    std::vector <int> Glm5MoeDsaModel::ForwardBatch(int batch, const fastllm::Data &inputIds, const fastllm::Data &attentionMask,
                            const fastllm::Data &positionIds, std::vector<std::pair<Data, Data>> &pastKeyValues,
                            const GenerationConfig &generationConfig, const LastTokensManager &lastTokens,
                            std::vector <std::vector <float>*> *retLogits) {
        Executor &excutor = *((Executor*)GetExecutor());
...
//inputIds.Print();
        ...
        for (int i = 0; i < block_cnt; i++) {
            ...
            RMSNorm(hiddenStates, this->weight["model.layers." + std::to_string(i) + ".input_layernorm.weight"],
                    rms_norm_eps, attenInput);
            Glm5MoeDsaPrintLayerData(attenInput, i, "Glm5MoeDsa ForwardBatch attn_norm");
            ...
            if (this->weight.weight.find(qaWeightName) != this->weight.weight.end()) {
                Linear(attenInputTemp, this->weight[qaWeightName], this->weight[qaBiasName], qa);
                Glm5MoeDsaPrintLayerData(qa, i, "Glm5MoeDsa ForwardBatch attn_q_a");
                RMSNorm(qa, this->weight[qRmsNormName], this->rms_norm_eps, qa);
                Glm5MoeDsaPrintLayerData(qa, i, "Glm5MoeDsa ForwardBatch attn_q_a_norm");
                Linear(qa, this->weight[qbWeightName], this->weight[qbBiasName], q);
            } else {
                Linear(attenInputTemp, this->weight[qWeightName], this->weight[qBiasName], q);
            }
            Glm5MoeDsaPrintLayerData(q, i, "Glm5MoeDsa ForwardBatch attn_q");
            ...
            Split(q, -1, qk_nope_head_dim, q_head_dim, q_pe);
            Linear(attenInputTemp, this->weight[compressedKvWeightName], this->weight[compressedKvBiasName], compressed_kv_ori);
            Glm5MoeDsaPrintLayerData(compressed_kv_ori, i, "Glm5MoeDsa ForwardBatch attn_kv_cmpr_pe");
            Split(compressed_kv_ori, -1, 0, kv_lora_rank, compressed_kv);
            Glm5MoeDsaPrintLayerData(compressed_kv, i, "Glm5MoeDsa ForwardBatch attn_kv_cmpr");
            Split(compressed_kv_ori, -1, kv_lora_rank, kv_lora_rank + qk_rope_head_dim, k_pe);
            RMSNorm(compressed_kv, this->weight[kvRmsNormName], this->rms_norm_eps, kv_ln);
            Glm5MoeDsaPrintLayerData(kv_ln, i, "Glm5MoeDsa ForwardBatch attn_kv_cmpr_norm");
            ...
            k_pe.Reshape({bsz, seqlen, 1, qk_rope_head_dim});
            Glm5MoeDsaPrintLayerData(k_pe, i, "Glm5MoeDsa ForwardBatch attn_k_pe");
            PermuteSelf(k_pe, {1, 0, 2, 3});
            fastllm::NearlyRotatePosition2D(k_pe, positionIds, *sinDataPtr, *cosDataPtr, rotary_dim);
            PermuteSelf(k_pe, {1, 0, 2, 3});
            PermuteSelf(k_pe, {0, 2, 1, 3});
            Glm5MoeDsaPrintLayerData(k_pe, i, "Glm5MoeDsa ForwardBatch attn_k_pe_rope");
...
// printf("matmul permuteself spend %f s.\n", GetSpan(st, std::chrono::system_clock::now()));
                ...
                MatMul(q_nope, kv0, result);
                Glm5MoeDsaPrintLayerHead0LastToken(result, i, "Glm5MoeDsa ForwardBatch attn_q_nope_absorbed");
// printf("matmul0 spend %f s.\n", GetSpan(st, std::chrono::system_clock::now()));
            ...
            Glm5MoeDsaPrintLayerData(attenOutput, i, "Glm5MoeDsa ForwardBatch kqv_out");
            Data oBias = (weight.weight.find(oBiasName) != weight.weight.end()) ? weight[oBiasName] : Data();
            if (Glm5MoeDsaShouldForceCpuOProj(i)) {
                Data cpuAttenOutput, cpuOWeight, cpuOBias;
                cpuAttenOutput.CopyFrom(attenOutput);
                cpuAttenOutput.ToDevice(DataDevice::CPU);
                cpuOWeight.CopyFrom(weight[oWeightName]);
                cpuOWeight.ggmlTensor = weight[oWeightName].ggmlTensor;
                cpuOWeight.disableGGUFRepack = true;
                cpuOWeight.ToDevice(DataDevice::CPU);
                if (oBias.dims.size() > 0) {
                    cpuOBias.CopyFrom(oBias);
                    cpuOBias.ToDevice(DataDevice::CPU);
                }
                Linear(cpuAttenOutput, cpuOWeight, cpuOBias, attenLastOutput);
                attenLastOutput.ToDevice(hiddenStates.dataDevice, hiddenStates.dataDeviceIds);
            } else {
                Linear(attenOutput, weight[oWeightName], oBias, attenLastOutput);
            }
            Glm5MoeDsaPrintLayerData(attenLastOutput, i, "Glm5MoeDsa ForwardBatch attn_out");

            AddTo(hiddenStates, attenLastOutput);
            Glm5MoeDsaPrintLayerData(hiddenStates, i, "Glm5MoeDsa ForwardBatch ffn_inp");
            RMSNorm(hiddenStates, this->weight["model.layers." + std::to_string(i) + ".post_attention_layernorm.weight"], rms_norm_eps, attenInput);
            Glm5MoeDsaPrintLayerData(attenInput, i, "Glm5MoeDsa ForwardBatch ffn_norm");
            // 2. moe mlp
            if (weight.weight.find("model.layers." + std::to_string(i) + ".mlp.gate_proj.weight") != weight.weight.end()) {
                if (Glm5MoeDsaShouldForceCpuDenseFFN(i)) {
                    Data cpuAttenInput, cpuGateWeight, cpuUpWeight, cpuDownWeight;
                    ...
                    Glm5MoeDsaCopyGGUFWeightForCpu(cpuDownWeight, weight["model.layers." + std::to_string(i) + ".mlp.down_proj.weight"]);
                    Linear(cpuAttenInput, cpuGateWeight, Data(), w1);
                    Glm5MoeDsaPrintLayerData(w1, i, "Glm5MoeDsa ForwardBatch ffn_gate");
                    Silu(w1, w1);
                    Glm5MoeDsaPrintLayerData(w1, i, "Glm5MoeDsa ForwardBatch ffn_gate_silu");
                    Linear(cpuAttenInput, cpuUpWeight, Data(), w3);
                    Glm5MoeDsaPrintLayerData(w3, i, "Glm5MoeDsa ForwardBatch ffn_up");
                    MulTo(w1, w3);
                    Glm5MoeDsaPrintLayerData(w1, i, "Glm5MoeDsa ForwardBatch ffn_swiglu");
                    Linear(w1, cpuDownWeight, Data(), w2);
                    w2.ToDevice(hiddenStates.dataDevice, hiddenStates.dataDeviceIds);
                } else if (CanRunLinearEx(LinearExType::ExSilu)) {
                    LinearEx(attenInput, weight["model.layers." + std::to_string(i) + ".mlp.gate_proj.weight"], Data(), w1, LinearExType::ExSilu);
                ...
                Glm5MoeDsaPrintLayerData(w2, i, "Glm5MoeDsa ForwardBatch ffn_out");
                AddTo(hiddenStates, w2);
            } else {
                // 这里是moe mlp
                std::string gateWeightName = "model.layers." + std::to_string(i) + ".mlp.gate.weight";
                std::string gateBiasName = "model.layers." + std::to_string(i) + ".mlp.gate.e_score_correction_bias";
                Glm5MoeDsaPrintOptionalWeight(this->weight, gateBiasName, i, "Glm5MoeDsa ForwardBatch ffn_moe_gate_bias");
                ...
                if (cudaSe) {
                    ...
                    if (sharedGateupIt != weight.weight.end() && sharedGateupIt->second.dims.size() > 0) {
                        Linear(attenInput, weight[sharedGateupName], Data(), ww3);
                        Glm5MoeDsaPrintLayerData(ww3, i, "Glm5MoeDsa ForwardBatch ffn_shexp_gateup");
                        Swiglu(ww3, ww1);
                    } else {
                        Linear(attenInput, weight["model.layers." + std::to_string(i) + ".mlp.shared_experts.gate_proj.weight"], Data(), ww3);
                        Glm5MoeDsaPrintLayerData(ww3, i, "Glm5MoeDsa ForwardBatch ffn_shexp_gate");
                        Silu(ww3, ww1);
                        Linear(attenInput, weight["model.layers." + std::to_string(i) + ".mlp.shared_experts.up_proj.weight"], Data(), ww3);
                        Glm5MoeDsaPrintLayerData(ww3, i, "Glm5MoeDsa ForwardBatch ffn_shexp_up");
                        MulTo(ww1, ww3);
                    }
                    Glm5MoeDsaPrintLayerData(ww1, i, "Glm5MoeDsa ForwardBatch ffn_shexp_swiglu");
                    Linear(ww1, weight["model.layers." + std::to_string(i) + ".mlp.shared_experts.down_proj.weight"], Data(), moeFinal2);
                    Glm5MoeDsaPrintLayerData(moeFinal2, i, "Glm5MoeDsa ForwardBatch ffn_shexp");
                    weights[i][0] = weights[i][1] = nullptr;
                }
                ...
                Glm5MoeDsaPrintLayerData(routerLogits, i, "Glm5MoeDsa ForwardBatch ffn_moe_probs");
                Data *gateBias = weight.weight.find(gateBiasName) != weight.weight.end() ? &weight[gateBiasName] : nullptr;
                Glm5MoeDsaPrintRouterBiasedTopK(routerLogits, gateBias, i, "Glm5MoeDsa ForwardBatch ffn_moe_probs_biased");
                ...
                if (weight.weight.find("model.layers." + std::to_string(i) + ".mlp.experts.0.gateup_proj.weight") != weight.weight.end()
                    ...
                    MergeMOEBlock(&attenInput, &expertIndex, &expertScore,
                        ...
                        &moeInputTemp, &moeOutputTemp);
                    if (cudaSe) {
                        Glm5MoeDsaPrintLayerData(moeFinal, i, "Glm5MoeDsa ForwardBatch ffn_moe_out");
                    }
                } else {
                    Data &bias = weight[gateBiasName];
                    ToDataType(routerLogits, DataType::FLOAT32);
                ...
                moeFinal.Reshape(hiddenStates.dims);
                if (!cudaSe) {
                    Glm5MoeDsaPrintLayerData(moeFinal, i, "Glm5MoeDsa ForwardBatch ffn_out");
                }
                ...
                if (cudaSe) {
                    moeFinal2.Reshape(hiddenStates.dims);
                    Glm5MoeDsaPrintLayerDataSum(moeFinal, moeFinal2, i, "Glm5MoeDsa ForwardBatch ffn_out");
                    AddTo(hiddenStates, moeFinal2);
                }
            }
            Glm5MoeDsaPrintLayerData(hiddenStates, i, "Glm5MoeDsa ForwardBatch l_out");
        }
        ...
        std::vector <int> lastRet;
        {
            auto &hiddenStates = *lastHiddenStates;
            RMSNorm(hiddenStates, weight["model.norm.weight"], rms_norm_eps, hiddenStates);
            Glm5MoeDsaPrintDataPrefix(hiddenStates, "Glm5MoeDsa ForwardBatch result_norm");
            Linear(hiddenStates, weight["lm_head.weight"], Data(), logits);
            ToDataType(logits, DataType::FLOAT32);
            ResetLogitsOfEOS(batch, &logits, pastKeyValues, generationConfig);
            Glm5MoeDsaPrintLogitsTopK(logits, batch, "Glm5MoeDsa ForwardBatch");
            if (Glm5MoeDsaGetString(this->weight, "model_type") != "glm_moe_dsa") {
                if (((GenerationConfig*)&generationConfig)->top_k <= 1) {
                    ((GenerationConfig*)&generationConfig)->top_k = 10;
                }
    ...
    std::vector <int> Glm5MoeDsaModel::ForwardBatch(int batch,
                                               const Data &inputIds,
        ...
        for (int i = 0; i < block_cnt; i++) {
            ...
            RMSNorm(hiddenStates, this->weight["model.layers." + std::to_string(i) + ".input_layernorm.weight"],
                    rms_norm_eps, attenInput);
            Glm5MoeDsaPrintLayerData(attenInput, i, "Glm5MoeDsa ForwardBatchBatch attn_norm");
            ...
            Data oBias = (weight.weight.find(oBiasName) != weight.weight.end()) ? weight[oBiasName] : Data();
            Linear(attenOutput, weight[oWeightName], oBias, attenLastOutput);
            Glm5MoeDsaPrintLayerData(attenLastOutput, i, "Glm5MoeDsa ForwardBatchBatch attn_out");
            AddTo(hiddenStates, attenLastOutput);
            Glm5MoeDsaPrintLayerData(hiddenStates, i, "Glm5MoeDsa ForwardBatchBatch ffn_inp");
            RMSNorm(hiddenStates, this->weight["model.layers." + std::to_string(i) + ".post_attention_layernorm.weight"], rms_norm_eps, attenInput);
            Glm5MoeDsaPrintLayerData(attenInput, i, "Glm5MoeDsa ForwardBatchBatch ffn_norm");
            // 2. moe mlp
            if (weight.weight.find("model.layers." + std::to_string(i) + ".mlp.gate_proj.weight") != weight.weight.end()) {
                if (Glm5MoeDsaShouldForceCpuDenseFFN(i)) {
                    Data cpuAttenInput, cpuGateWeight, cpuUpWeight, cpuDownWeight;
                ...
                Glm5MoeDsaPrintLayerData(w2, i, "Glm5MoeDsa ForwardBatchBatch ffn_out");
                AddTo(hiddenStates, w2);
            } else {
                // 这里是moe mlp
                std::string gateWeightName = "model.layers." + std::to_string(i) + ".mlp.gate.weight";
                std::string gateBiasName = "model.layers." + std::to_string(i) + ".mlp.gate.e_score_correction_bias";
                Glm5MoeDsaPrintOptionalWeight(this->weight, gateBiasName, i, "Glm5MoeDsa ForwardBatchBatch ffn_moe_gate_bias");
                ...
                if (cudaSe) {
                    ...
                    Linear(ww1, weight["model.layers." + std::to_string(i) + ".mlp.shared_experts.down_proj.weight"], Data(), moeFinal2);
                    Glm5MoeDsaPrintLayerData(moeFinal2, i, "Glm5MoeDsa ForwardBatchBatch ffn_shexp");
                    weights[i][0] = weights[i][1] = nullptr;
                }
                ...
                Glm5MoeDsaPrintLayerData(routerLogits, i, "Glm5MoeDsa ForwardBatchBatch ffn_moe_probs");
                Data *gateBias = weight.weight.find(gateBiasName) != weight.weight.end() ? &weight[gateBiasName] : nullptr;
                Glm5MoeDsaPrintRouterBiasedTopK(routerLogits, gateBias, i, "Glm5MoeDsa ForwardBatchBatch ffn_moe_probs_biased");
                ...
                if (weight.weight.find("model.layers." + std::to_string(i) + ".mlp.experts.0.gateup_proj.weight") != weight.weight.end()
                    ...
                    MergeMOEBlock(&attenInput, &expertIndex, &expertScore,
                        ...
                        &moeInputTemp, &moeOutputTemp);
                    if (cudaSe) {
                        Glm5MoeDsaPrintLayerData(moeFinal, i, "Glm5MoeDsa ForwardBatchBatch ffn_moe_out");
                    }
                } else {
                    Data &bias = weight[gateBiasName];
                    ToDataType(routerLogits, DataType::FLOAT32);
                ...
                moeFinal.Reshape(hiddenStates.dims);
                if (!cudaSe) {
                    Glm5MoeDsaPrintLayerData(moeFinal, i, "Glm5MoeDsa ForwardBatchBatch ffn_out");
                }
                ...
                if (cudaSe) {
                    moeFinal2.Reshape(hiddenStates.dims);
                    Glm5MoeDsaPrintLayerDataSum(moeFinal, moeFinal2, i, "Glm5MoeDsa ForwardBatchBatch ffn_out");
                    AddTo(hiddenStates, moeFinal2);
                }
            }
            Glm5MoeDsaPrintLayerData(hiddenStates, i, "Glm5MoeDsa ForwardBatchBatch l_out");
        }
        ...
        RMSNorm(hiddenStates, weight["model.norm.weight"], rms_norm_eps, hiddenStates);
        Glm5MoeDsaPrintDataPrefix(hiddenStates, "Glm5MoeDsa ForwardBatchBatch result_norm");
        Linear(hiddenStates, weight["lm_head.weight"], Data(), logits);
        ToDataType(logits, DataType::FLOAT32);
        std::vector <int> lastRet;

        ResetLogitsOfEOS(batch, &logits, pastKeyValues, generationConfigs);
        Glm5MoeDsaPrintLogitsTopK(logits, batch, "Glm5MoeDsa ForwardBatchBatch");
        if (Glm5MoeDsaGetString(this->weight, "model_type") != "glm_moe_dsa") {
    ...
    bool Glm5MoeDsaModel::NeedAttentionMask(int qlen, int klen) {
        if (((qlen == 1) || (qlen >= 8192))) {
            return false;
        }
        return true;
    ...
    void Glm5MoeDsaModel::FillLLMInputsBatch(std::vector<std::vector<float>> &inputTokens,
        ...
        if (index == 0) {
            ...
            for (int i = 0; i < batch; i++) {
                ...
                int promptLen = len;
                auto promptLenIt = params[i].find("promptLen");
                if (promptLenIt != params[i].end()) {
                    promptLen = promptLenIt->second;
                }
                int positionBase = std::max(0, promptLen - len);
                for (int j = 0; j < len; j++) {
                    vpids[i * maxLen + base + j] = positionBase + j;
                }
    ...
    std::string Glm5MoeDsaModel::MakeInput(const std::string &history, int round, const std::string &input) {
        return (round == 0 ? pre_prompt : history) + user_role + input + bot_role;
    }

    std::string Glm5MoeDsaModel::MakeHistory(const std::string &history, int round, const std::string &input, const std::string &output) {
        return (round == 0 ? pre_prompt : history) + user_role + input + bot_role + output + history_sep;
    }

    void Glm5MoeDsaModel::WarmUp() {
        printf("Warmup...\n");
